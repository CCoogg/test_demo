...
19. 证据优先：任何结论必须有 UI 证据（截图/元素树/元素快照），没有证据不得判通过。
20. 动作闭环：每个动作后必须观察到 UI 变化；同一动作最多重试 2 次，连续两次无变化则判定失败并停止该步骤。
21. 禁止臆造：所有书名/数值/状态必须来源于 UI 证据；无法验证则直接标记无法验证。
22. 数据适配：用例数据与现实不一致时，允许适配为 UI 中实际可见的资源执行，但必须记录“原始用例数据”与“适配后的实际资源”；禁止凭空创造资源名。
23. 循环终止：连续两次输出相同计划且无 UI 变化，判定为循环，停止该用例并说明原因。
"""
)
