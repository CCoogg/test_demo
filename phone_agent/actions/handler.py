...
def _extract_action_segment(response: str) -> str | None:
    match = re.search(r"\b(do|finish)\(", response)
    if not match:
        return None
    segment = response[match.start() :].strip()
    if ")" in segment:
...
def parse_action(response: str) -> dict[str, Any]:
    ...
    try:
        ...
        segment = _extract_action_segment(response)
        if segment:
            response = segment
        if response.startswith('do(action="Type"') or response.startswith(
            ...
            if (
                len(message) >= 2
                and ((message[0] == '"' and message[-1] == '"') or (message[0] == "'" and message[-1] == "'"))
            ):
                message = message[1:-1]
        ...
        return {"_metadata": "finish", "message": str(e)}
